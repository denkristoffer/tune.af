...
class Player extends PureComponent {
  ...
  requestAudio() {
    ...
    this.soundCloudAudio.resolve(resolveUrl, data => {
      ...
      this.setState({ playlist }, () => {
        ...
        location
          ? this.playTrackByPermalink(location)
  ...
  playTrackByPermalink(permalink: string) {
    ...
    if (playlist) {
      const chosenTrack = pickBy(
        playlist,
        track => track.permalink === permalink
      );
  ...
  handlePlayPauseClick() {
    ...
    if (playing) {
      ...
      this.soundCloudAudio.play({
        playlistIndex: this.soundCloudAudio._playlistIndex,
      });
    }
      ...
      <div>
        <Helmet
          link={[
            {
              rel: 'icon',
              href: playing ? `${Favicon32}` : `${FaviconPaused32}`,
              sizes: '32x32',
            },
            {
              rel: 'icon',
              href: playing ? `${Favicon16}` : `${FaviconPaused16}`,
              sizes: '16x16',
            },
          ]}
        />
